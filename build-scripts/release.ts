...
const package_json = JSON.parse(fs.readFileSync("package.json", "utf-8")) as { version: string; dependencies: Record<string, string>; name: string; };
...
function copy_release_file (file: string, dest?: string) {
	fs.copyFileSync(file, path.join(release_dir, dest ?? path.basename(file)));
}

function copy_release_dir (dir: string, dest?: string, args?: fs.CopySyncOptions) {
	fs.cpSync(dir, path.join(release_dir, dest ?? path.basename(dir)), { recursive: true, ...args });
}
